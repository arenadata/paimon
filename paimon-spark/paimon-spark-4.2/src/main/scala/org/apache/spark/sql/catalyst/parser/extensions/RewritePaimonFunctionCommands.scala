...
import org.apache.spark.sql.catalyst.analysis.{UnresolvedFunction, UnresolvedFunctionName, UnresolvedIdentifier}
...
case class RewritePaimonFunctionCommands(spark: SparkSession)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    // Needs to be done here and transform to `UnResolvedPaimonV1Function`, so that spark's
    // Analyzer can resolve the 'arguments' without throwing an exception, saying that function is
    // not supported.
          ...
          case u: UnresolvedFunction =>
            CatalogAndFunctionIdentifier.unapply(u.nameParts) match {
              case Some((v1FunctionCatalog: SupportV1Function, funcIdent, false))
                  // For Paimon built-in functions, Spark will resolve them by itself.
                  if !isPaimonBuildInFunction(funcIdent) =>
                // If the function is already registered, avoid redundant lookup in the catalog.
  ...
  private def isSparkBuiltInFunction(funcIdent: FunctionIdentifier): Boolean = {
    // Spark 4.2 SessionCatalog API takes function name instead of FunctionIdentifier.
    catalogManager.v1SessionCatalog.isBuiltinFunction(funcIdent.funcName)
  }
